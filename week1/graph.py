...
class Graph(object):
    ...
    def __add__(self, other: "Graph") -> "Graph":
        ...
        res = Graph(False)
        temp = dict()
        for u, v in zip(self.vertices, other.vertices):
            newU, newV = Vertex(res), Vertex(res)
            print("u: " + str(newU) + "\nv: " + str(newV))
            temp[u], temp[v] = newU, newV
            res.add_vertex(newU)
            res.add_vertex(newV)
        for e1, e2 in zip(self.edges, other.edges):
            res.add_edge(Edge(temp[e1.tail], temp[e1.head]))
            res.add_edge(Edge(temp[e2.tail], temp[e2.head]))
        return res
